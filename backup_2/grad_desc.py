...
if __name__ == "__main__": 	
	rawdata=fi.read_file("other_thing_31to38_25to100.csv", ",")
	lraw=rawdata[:,2]
	braw=rawdata[:,3]
	rraw=rawdata[:,4]
	vgsrraw=rawdata[:,5]
...
def MakeOrbit(x):
  ...
  while chivgsrindex < npoints:
    chivgsrnum = np.subtract(vgsrmodelarray[chivgsrindex],obsdata[chivgsrindex,2])
  ...
  while chirindex < npoints:
    chirnum = np.subtract(rmodelarray[chirindex],obsdata[chirindex,3])
    chirdenom = rerror
...
x0 = np.asarray((43.06,35.13,-46.99,113.24,-5.17))
res1 = optimize.fmin_cg(MakeOrbit, x0, gtol = 1e-5, epsilon = 1e-02)
