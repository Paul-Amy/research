...
os.system("touch grad_desc_output")
...
n_iter = int(raw_input("How many iterations? "))
while iter_count < n_iter:
  ...
  vgsrraw=rawdata[:,5]
  fit_params(lraw,braw,rraw,vgsrraw)
  b_rand = random.uniform(44.0,46.0)
  r_rand = random.uniform(11.0,12.0)
  vx_rand = random.uniform(-110.0,-90.0)
  vy_rand = random.uniform(90.0,110.0)
  vz_rand = random.uniform(90.0,110.0)
  x0 = np.asarray((b_rand,r_rand,vx_rand,vy_rand,vz_rand))
  print x0
  res1 = optimize.fmin_cg(MakeOrbit, x0, gtol = 1e-05, epsilon = 1e-02)
  chisqstr_out = MakeOrbit(res1)
  output = open("grad_desc_output", "a")
  output.write(str(res1) + ", " + str(chisqstr_out) + "\n")
  output.close()
