...
def get_params(x,y,z,vx,vy,vz):
	...
	vgsr=(xsol*vx+y*vy+z*vz)/rsol
	l=(np.arctan2(y,xsol))*rad2deg
	i = 0
	for i in range(len(x)):
	  if l[i] < 0:
	    l[i] = l[i] + 360.0
	b=(np.arctan2(z,np.sqrt(np.power(xsol,2)+np.power(y,2))))*rad2deg
	...
	for j in range(len(x)):
	  ...
	  params.write(str(x[j]) + "," + str(y[j]) + "," + str(z[j]) + "," + str(vx[j]) + "," + str(vy[j]) + "," + str(vz[j]) + "," + str(rgal[j]) + "," + str(vgal[j]) + "," + str(rsol[j]) + "," + str(vgsr[j]) + "," + str(l[j]) + "," + str(b[j]) + "\n")
	params.close()
